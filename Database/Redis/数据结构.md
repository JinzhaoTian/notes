...
其中key值应该是就是一个名字，字符指定。​ 其中list、hash、set、zset称为容器型数据结构，容器型数据结构有两条通用规则：
- 如果容器不存在，就创建一个，再进行操作。
...
string 是 Redis 最简单的数据结构，一个 key 对应一个 value 。Redis 的 string 实际上是使用数组实现的。
