...
1. **核心进程（后端）**：由 Rust 编写，是应用的入口，是唯一拥有直接访问操作系统底层能力的部分，负责窗口管理、文件系统访问等。
2. **WebView 进程（前端）**：被隔离在类似浏览器的沙盒中，负责通过系统自带的 WebView 渲染你的 HTML/CSS/JS 代码，构建用户界面。

两部分之间通过进程间通信（IPC）进行消息传递，前端使用 JavaScript 调用后端提供的功能，实现了前后端的分离。

### 前后端分离

Tauri 采用前后端分离的结构，主要是出于对安全性、稳定性和灵活性的核心追求。

1. **安全性**：核心驱动力
	- **隔离与最小权限**：Tauri应用由核心进程（后端）和WebView进程（前端）组成。核心进程是唯一能完全访问操作系统的部分，前端则被隔离在类似浏览器的沙盒中。这遵循了**最小权限原则**，即只给每个部分完成任务所需的最小权限。即使前端被攻破，攻击者也无法直接获取系统权限，将潜在危害降到最低
	- **敏感操作在后端**：所有敏感数据和核心业务逻辑都应放在Rust后端处理。前端只负责展示，通过**进程间通信（IPC）** 与后端交互，避免了将机密信息暴露在前端代码中。

2. **稳定性与性能**：各司其职
	- **防止UI卡顿**：在早期单进程GUI应用中，繁重计算会阻塞界面，导致卡顿甚至崩溃。Tauri的多进程架构将UI渲染（前端）和业务逻辑（后端）分离，即使后端执行密集型任务，前端界面也能保持流畅响应。
	- **提升容错性**：由于组件运行在不同进程，一个进程的崩溃不会导致整个应用宕机，系统可以单独重启出错的进程。
	- **发挥Rust优势**：选择性能高且内存安全的Rust来实现后端，非常适合处理敏感的系统级操作和复杂逻辑。

3. **灵活性与开发效率**：发挥各自所长
	- **前端技术自由**：开发者可以自由选择任何现代前端框架（如React、Vue等）来构建界面[](https://v2.tauri.org.cn/about/philosophy/)，Web技术生态的丰富工具和库都能直接使用。
	- **跨平台UI**：Tauri利用操作系统自带的WebView渲染引擎（如Windows的WebView2），既能保证原生体验，又能显著减小应用体积。
	- **后端语言可扩展**：虽然目前是Rust，但Tauri的设计哲学是开放的，未来可能支持Go、Python等其他语言作为后端。

4. **资源效率**：轻量化的基础

这种分离也是Tauri实现极致轻量的关键。应用体积之所以能小至5MB左右，正是因为其前端UI直接调用了操作系统自带的WebView库，而无需像Electron那样打包一个完整的Chromium浏览器。
